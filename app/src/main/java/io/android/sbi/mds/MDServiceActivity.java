...
import io.android.sbi.device.CaptureActivity;
...
public class MDServiceActivity extends AppCompatActivity {
    ...
    private void capture(byte[] input, DeviceConstants.BioType bioType) {
        try {
            ...
            //Validations
            List<CaptureRequestDeviceDetailDto> bioRequest = captureRequestDto.bio;

            int deviceSubId = Integer.parseInt(bioRequest.get(0).deviceSubId);
            int count = Integer.parseInt(bioRequest.get(0).count);

            if (!validateBioCountAuth(bioType, count, bioRequest.get(0))) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        CaptureResponse captureResponse = responseGenHelper
                                .getCaptureBiometrics(captureResult, captureRequestDto, keystore);
